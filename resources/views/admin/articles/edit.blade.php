...
@section('title', 'แก้ไขบทความ - EngBuddy Admin')
...
<div class="form-container">
    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label for="title">
                ชื่อบทความ *
                <span class="info-badge">ยอดดู: {{ number_format($article->views) }}</span>
            </label>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <p class="help-text">URL: /student/articles/{{ $article->slug }}</p>
                @error('slug')
                    <p style="color: #e74c3c; font-size: 13px; margin-top: 5px;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="category" name="category" class="form-control" value="{{ old('category', $article->category) }}" placeholder="เช่น: เรียนภาษาอังกฤษ">
                @error('category')
                    <p style="color: #e74c3c; font-size: 13px; margin-top: 5px;">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="form-group">
            ...
            <textarea id="short_description" name="short_description" class="form-control" rows="3" placeholder="สรุปบทความสั้นๆ (สูงสุด 500 ตัวอักษร)">{{ old('short_description', $article->short_description) }}</textarea>
        ...
        <div class="form-group">
            ...
            <textarea id="content" name="content" class="form-control" rows="15" required>{{ old('content', $article->content) }}</textarea>
            @error('content')
                <p style="color: #e74c3c; font-size: 13px; margin-top: 5px;">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            @if($article->image)
                <div class="current-image-section">
                    ...
                    <img src="{{ asset($article->image) }}" alt="{{ $article->title }}" class="current-image">
                </div>
            @endif
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="author" name="author" class="form-control" value="{{ old('author', $article->author) }}" placeholder="ชื่อผู้เขียน">
                @error('author')
                    <p style="color: #e74c3c; font-size: 13px; margin-top: 5px;">{{ $message }}</p>
                @enderror
            </div>
